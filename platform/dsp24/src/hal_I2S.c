...
void set_I2S_params(int channel, int tx_en, int rx_en, int bitdepth_tx, int bitdepth_rx, int clkgen, int dacen, int ws_len) {
    uint16_t params = *(uint16_t*)(I2S_BASE + channel*0x2);
    ...
    *(uint16_t*)(I2S_BASE + channel*0x2) = params;
}

void set_I2S_clkdiv(int channel, int clkdiv) {
    *(uint16_t*)(I2S_CLKDIV + (channel*0x2)) = clkdiv;
}

void set_I2S_watermark(int channel, int watermark_tx, int watermark_rx) {
    *(uint8_t*)(I2S_TX_WATERMARK + channel) = watermark_tx;
    *(uint8_t*)(I2S_RX_WATERMARK + channel) = watermark_rx;
}
...
uint64_t read_I2S_tx(int channel, int left) {
    if (left)
        return *(uint64_t*)(I2S_TX_L + (channel * 0x10));
    else
        return *(uint64_t*)(I2S_TX_R + (channel * 0x10));
}

void write_I2S_rx(int channel, int left, uint64_t data) {
    if (left)
        *(uint64_t*)(I2S_RX_L + (channel * 0x10)) = data;
    else
        *(uint64_t*)(I2S_RX_R + (channel * 0x10)) = data;
}
...
void set_I2S_fp(int channel, int tx_fp, int rx_fp) {
    ...
    *(uint16_t*)(I2S_BASE + channel) = params;
}
